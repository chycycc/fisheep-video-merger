...
|---|------|------|
| 5.2.1 | extract 面板激活 | ✅ |
| 5.2.x | extract-format 可操作 | ✅ |
| 5.2.x | extract-bitrate-mode 可操作 | ✅ |
| 5.2.x | extract-bitrate 可操作 | ✅ |
| 5.2.x | extract-channels 可操作 | ✅ |
| 5.2.x | extract-sample-rate 可操作 | ✅ |
| 5.2.x | extract-volume 可操作 | ✅ |
| 5.2.2 | 预设按钮存在 | ✅ |
...
## Phase 6: Bug 验证测试详情 (3/3)

| # | 用例 | 结果 | 说明 |
|---|------|------|------|
| 6.1 | Hash 路由 #subtitle | ✅ | 已修复，导航正常 |
| 6.2 | 控制台无 ReferenceError | ✅ | 无 JS 错误 |
| 6.3 | 无残留旧 ID | ✅ | subtitle.js 中无旧 ID |
...
| 5 | Hash 路由缺少 subtitle | js/main.js | validTools 数组添加 'subtitle' | `520c182` |
...
| # | 问题 | 严重度 | 说明 |
|---|------|--------|------|
| 1 | initSettingsPanel 引用不存在的 DOM ID | 低 | 设置面板部分功能未完全实现 |
| 2 | action-bar 响应式问题 | 低 | `window.toolFiles` 非 Alpine 响应式对象 |
| 3 | subtitle-adjust-segments API 无 JS 调用 | 低 | API 存在但前端未暴露 |
| 4 | showToastWithAction 闭包问题 | 低 | 复杂函数 toString 可能失败 |
| 5 | 文件类型过滤不一致 | 低 | 合并对话框和工具对话框支持的格式不同 |
...
**建议**：5 个已知问题均为低严重度，不影响核心功能，可在后续版本修复。
