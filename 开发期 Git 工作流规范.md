...
为了完美应对开发中多变的信息负载量，我们正式确立两类互补的提交风格。您可以根据本次提交的“综合体量”来科学决策选用：

#### 🍃 风格 A：灵动单行型（Compact Single-line Form）
*   **模板结构**：`类型(模块): 一句话精准陈述改动核心`
*   **命令行实操**：`git commit -m "fix(ui): 修正侧栏折叠按钮文字在明亮模式下的超界问题"`
*   **最佳应用场景**：
    1. **原子化改动**：本次仅修补了一个笔误、重构了单行逻辑，或调校了单个控件的视觉间距。
    2. **高自解释性**：改动极度单纯，一句话即代表了事实的全部，无需追溯复杂的上下文。
    3. **日常微备份**：作为在 `dev` 分支高频开发的临时停靠点，为避免丢失进度而做的快照。

#### 🌲 风格 B：结构化多行型（Structured Multiline Form）——【极力推崇！】
*   **模板结构**：
    ```text
    主标题: 一句话高度概括本组连环改动的最高意图
    
    - 连环动作 1：细粒度展开，刻画具体的功能扩充细节
    - 连环动作 2：指引与之匹配的周边联动修改或前置策略
    - 配套归集：注明关联的优化案代号（如 S-5, U-4），帮助全链路追溯
    ```
*   **命令行实操**：
    ```bash
    git commit -m "feat: 实现已完整文件展示、转封装、快捷键支持及多项体验优化" \
               -m "- 实现「已完整文件」展示标签页，支持批量转封装到 mp4/mkv (M-3, V-3)" \
               -m "- 支持单文件添加与拖拽 .m4s 文件支持 (M-4)" \
               -m "- 降级 PySide6 至 6.9.3 解决崩溃并锁定依赖 (F-1, D-4)"
    ```
*   **最佳应用场景**：
    1. **会战大捷式合并**：一次性交付了一整套互相关联的逻辑包（比如“侧栏看板+表格拉伸”组合拳）。
    2. **重大布局重构**：底层动刀较多，需要向审查者（或未来的自己）提供一览全局的“改动大图纸”。
    3. **阶段性里程碑**：作为功能合拢、发布前夕，或拉取 PR/MR 时最完美的交付报告单，方便日后自动化生成 changelog 文档。

#### 📊 两种提交流派对阵沙盘

| 考核维度 | 🍃 风格 A（灵动单行） | 🌲 风格 B（结构化多行） |
| :--- | :--- | :--- |
| **信息承载力** | **单一视角**：聚焦于“做了什么”。 | **立体航拍**：从“是什么”深入到“怎么做”和“为什么”。 |
| **首屏阅读成本** | 零负荷，如同浏览目录。 | 详实底蕴，提供清晰的问题归纳。 |
| **触发典型频次** | 日常开发高频穿插（日均 3-5 次）。 | 功能合龙、日末总结、或重大架构迭代完毕。 |
| **Git 日志观感** | 如溪流般敏捷清脆，适合寻找细分切入点。 | 如史书般厚重严谨，每一页都是一篇开发简报。 |
